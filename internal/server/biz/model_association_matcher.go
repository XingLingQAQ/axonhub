...
func matchChannelModel(assoc *objects.ModelAssociation, channels []Channel) []*ModelChannelConnection {
	...
	modelEntries := ch.GetModelEntries()
	supportedModels := lo.Map(modelEntries, func(e ChannelModelEntry, _ int) string {
		return e.RequestModel
	})

	if !lo.Contains(supportedModels, assoc.ChannelModel.ModelID) {
		return nil
	}
...
func matchChannelRegex(assoc *objects.ModelAssociation, channels []Channel) []*ModelChannelConnection {
	...
	modelIds := xregexp.FilterByPattern(supportedModels, assoc.ChannelRegex.Pattern)
	if len(modelIds) == 0 {
		return nil
	}
...
func matchRegex(assoc *objects.ModelAssociation, channels []Channel) []*ModelChannelConnection {
	...
	for _, ch := range channels {
		...
		modelIds := xregexp.FilterByPattern(supportedModels, assoc.Regex.Pattern)
		if len(modelIds) == 0 {
			continue
		}
...
func matchModel(assoc *objects.ModelAssociation, channels []Channel) []*ModelChannelConnection {
	...
	for _, ch := range channels {
		...
		if !lo.Contains(supportedModels, modelID) {
			continue
		}
